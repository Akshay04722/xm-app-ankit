      ...
      <div className="container mx-auto px-4 mt-4">
        <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
          ...
          <div className="w-full lg:w-2/3">
            ...
            <div className="hidden md:grid grid-cols-12 bg-[#F9F1E7] py-3.5 px-6 rounded-lg text-xs font-semibold uppercase tracking-wider text-gray-700 mb-6">
              ...
              {cartItems.map((item) => {
                ...
                return (
                  <div key={`${item.sku}-${item.selectedColor || ""}-${item.selectedSize || ""}`}>
                    {/* Desktop View */}
                    <div className="hidden md:grid grid-cols-12 items-center py-1.5 px-6 text-sm text-gray-900 border-b border-gray-100 pb-5">
                      <div className="col-span-6 flex items-center gap-6">
                        ...
                        <div className="flex flex-col gap-0.5 pr-4">
                          <span className="font-normal text-[14px] text-gray-500" title={item.title}>
                            {item.title}
                          </span>
                      ...
                      <div className="col-span-2 text-center text-[14px] text-gray-600 font-normal">
                      ...
                      <div className="col-span-2 flex justify-center">
                        <div className="flex items-center border border-gray-300 rounded-md bg-white overflow-hidden">
                          <button
                            ...
                            className="px-2 py-0.5 text-gray-500 hover:bg-gray-50 font-semibold active:scale-95 transition-all cursor-pointer text-[14px]"
                          >
                            −
                          </button>
                          <span className="w-8 text-center text-gray-900 text-xs font-medium select-none">
                            {item.quantity}
                          </span>
                          <button
                            type="button"
                      ...
                      <div className="col-span-2 text-right text-[14px] text-gray-900 font-medium">
                    ...
                    <div className="md:hidden flex gap-4 p-4 border border-gray-150 rounded-xl relative bg-white shadow-xs">
                      ...
                      <div className="flex flex-col justify-between flex-1 py-0.5">
                        <div>
                          <h4 className="font-medium text-[#242424] text-[14px] line-clamp-1 pr-6">{item.title}</h4>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center border border-gray-300 rounded-md bg-white">
                            ...
                            <span className="w-6 text-center text-xs text-gray-900 font-semibold">{item.quantity}</span>
                            <button
                              type="button"
                              onClick={() => updateQuantity(item.sku, item.quantity + 1, item.selectedColor, item.selectedSize)}
                              className="px-2 py-0.5 text-gray-500 font-bold"
                          ...
                          <div className="text-right flex flex-col">
                            <span className="text-[9px] text-gray-400">Subtotal:</span>
                            <span className="text-xs font-semibold text-[#B88E2F]">{formatPrice(activePrice * item.quantity)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center mt-8 pt-6 border-t border-gray-100">
              <form onSubmit={handleApplyCoupon} className="flex gap-3 w-full sm:w-auto">
                <input
                  ...
                  className="border border-gray-300 rounded-lg px-3.5 py-2 text-[13px] w-full sm:w-44 focus:outline-none focus:border-[#B88E2F] disabled:bg-gray-50 disabled:text-gray-400"
                />
                <button
                  type="submit"
                  disabled={couponApplied}
                  className="border border-black hover:bg-black hover:text-white text-black font-normal px-5 py-2 rounded-lg text-[13px] transition-all duration-200 cursor-pointer whitespace-nowrap active:scale-95 disabled:border-gray-300 disabled:text-gray-400 disabled:bg-transparent disabled:cursor-not-allowed"
                >
              ...
              <button
                type="button"
                className="border border-gray-300 text-gray-400 font-normal px-5 py-2 rounded-lg text-[13px] transition-all cursor-not-allowed w-full sm:w-auto bg-gray-50"
                disabled
              >
                Update cart
              </button>
          ...
          <div className="bg-[#F9F1E7] rounded-none px-6 py-8 flex flex-col justify-between h-fit w-full lg:w-1/3 mt-8 lg:mt-0 shadow-xs border border-[#f4ebdf]">
            <div className="w-full">
              <h2 className="text-[24px] font-bold text-gray-900 text-center mb-6">Cart Totals</h2>
              
              <div className="flex justify-between items-center pb-4 mb-4 border-b border-[#ebdcca] px-2">
                <span className="text-[14px] font-semibold text-gray-900">Subtotal</span>
                <span className="text-[14px] text-gray-400 font-medium">{formatPrice(subtotal)}</span>
              </div>
              
              <div className="flex justify-between items-center mb-8 px-2">
                <span className="text-[14px] font-semibold text-gray-900">Total</span>
                <span className="text-[18px] font-bold text-[#B88E2F]">{formatPrice(subtotal)}</span>
              </div>
            </div>
            ...
            <div className="w-full flex justify-center mt-2">
              {user ? (
                <button
                  ...
                  className="w-full bg-black hover:bg-[#B88E2F] border border-black hover:border-[#B88E2F] text-white py-3 px-6 rounded-xl text-[14px] font-semibold transition-all duration-200 cursor-pointer text-center active:scale-98"
                >
                  Proceed to checkout
                </button>
              ) : (
                <Link
                  ...
                  className="w-full bg-black hover:bg-[#B88E2F] border border-black hover:border-[#B88E2F] text-white py-3 px-6 rounded-xl text-[14px] font-semibold transition-all duration-200 cursor-pointer text-center active:scale-98 block decoration-none"
                >
